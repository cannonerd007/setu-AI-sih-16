...
import {
  ...
  Settings, ShieldCheck, SlidersHorizontal, Sparkles, Volume2, Waves, WifiOff, X
} from 'lucide-react'

const maroon = '#7a3226'
const tabs = [
  { id: 'home', label: 'होम', icon: Home }, { id: 'translator', label: 'अनुवादक', icon: Waves },
  { id: 'curriculum', label: 'पाठ्यक्रम', icon: BookOpen }, { id: 'worksheets', label: 'कार्यपत्रक', icon: FileText },
...
function HomeScreen({ go }: { go: (s: string) => void }) {
  return <><div className="language-switch"><button className="active">संथाली</button><button>हिन्दी</button><ArrowLeftRight size={22} /></div>
    <section className="hero-card"><div><span className="eyebrow">कक्षा में तुरंत सहायता</span><h1>कक्षा अनुवाद</h1><p>सीधा बोलकर अनुवाद करें</p><button className="hero-cta" onClick={() => go('translator')}><Mic size={19} /> बोलना शुरू करें</button><small>माइक दबाकर बोलें</small></div><div className="hero-orb"><Mic size={48} /></div></section>
    <SectionTitle action="सभी पाठ">आज का पाठ</SectionTitle><section className="lesson-card"><div className="lesson-image"><div className="sun" /><div className="hut" /><div className="tree" /></div><div className="lesson-content"><span className="tag">भाषा • कक्षा ३</span><h3>हमारा सुंदर गाँव</h3><p>अपने गाँव और आसपास की चीज़ों के बारे में सीखें।</p><div className="lesson-actions"><button className="primary-btn" onClick={() => go('curriculum')}>पाठ शुरू करें</button><button className="secondary-btn"><BookOpen size={16} /> शब्दावली</button></div></div></section>
    <SectionTitle action="और देखें">त्वरित भाषा सामग्री</SectionTitle><div className="media-row"><MediaCard kind="बालगीत" title="चलो गाएँ" icon="♫" /><MediaCard kind="कहानी" title="चतुर चिड़िया" icon="◉" /></div>
...
function SettingsScreen() { return <><div className="profile-card"><div className="profile-avatar">अशि</div><div><h2>आशा मुर्मू</h2><p>सहायक शिक्षक • प्राथमिक विद्यालय</p><small>सत्र २०२५–२६</small></div><span className="verified"><ShieldCheck size={14} /> सत्यापित संकुल</span></div><SectionTitle>भाषा विस्तार रोडमैप</SectionTitle><div className="roadmap">{[['संथाली','तैयार और सक्रिय','green'],['हो','प्रशिक्षण जारी','amber'],['मुंडारी','डेटा संग्रह जारी','gray']].map(x => <div><span className={`road-dot ${x[2]}`} /><strong>{x[0]}</strong><small>{x[1]}</small><ChevronRight size={16} /></div>)}</div><section className="contribution-card"><span className="eyebrow">शिक्षक सहभागिता</span><h2>अपनी बोली से सेतु बनाएँ</h2><p>मॉडल को बेहतर बनाने के लिए ५ छोटे वाक्य रिकॉर्ड करें।</p><div className="sample-line">“बच्चों, आज हम पेड़ के बारे में सीखेंगे।”</div><button className="record-btn"><Mic size={18} /> अपनी आवाज़ जोड़ें</button></section><SectionTitle>पठन व ध्वनि प्राथमिकताएं</SectionTitle><div className="preference-row"><span><SlidersHorizontal size={18} /> अक्षर का आकार</span><b>सामान्य　/　बड़ा आकार</b></div><div className="preference-row"><span><Mic size={18} /> माइक शोर निवारक</span><div className="switch on"><i /></div></div><div className="help-row"><CircleHelp size={20} /><div><strong>सहायता एवं संकुल संपर्क</strong><small>समन्वयक: रमेश हेम्ब्रम • ☎ 1800-123-SETU</small></div><ChevronRight size={17} /></div><footer>SETU सेतु • संस्करण 1.0.0 (डेमो)</footer></> }

export default function Page() { const [screen, setScreen] = useState('home'); const [settings, setSettings] = useState(false); const titles: Record<string,string> = { home:'आपका आज का शिक्षण साथी', translator:'बोलें और सीखें', curriculum:'पाठ और वाक्य अभ्यास', worksheets:'देखें, बोलें, सीखें', storage:'आपकी सामग्री', settings:'सेटिंग्स' }; const active = settings ? 'settings' : screen; return <div className="tablet-shell"><StatusStrip /><Header title={titles[active]} onSettings={() => setSettings(true)} /><main className="screen-content">{active==='home'&&<HomeScreen go={s=>{setScreen(s);setSettings(false)}} />}{active==='translator'&&<TranslatorScreen />}{active==='curriculum'&&<CurriculumScreen go={s=>setScreen(s)} />}{active==='worksheets'&&<WorksheetsScreen />}{active==='storage'&&<StorageScreen />}{active==='settings'&&<SettingsScreen />}</main><nav className="bottom-nav">{tabs.map(t => { const Icon=t.icon; return <button className={active===t.id?'active':''} onClick={()=>{setScreen(t.id);setSettings(false)}} key={t.id}><Icon size={21}/><span>{t.label}</span></button>})}</nav></div> }

function _unused() { return <><Menu/><LockKeyhole/><MoreHorizontal/><X/></> }
